...
def image_thread():
    ...
    while not kill:
        ...
        for x in range(width):
            ...
            for y in range(height):
                ...
                if column[y] == 255:
                    ...
                    if count >= 20:
                        # Feketítsük ki a start feletti részt
                        frame[:start + count + 10, x] = 0
                        hsv[:start + count + 10, x] = 0
                        #break
                    count = 0
                    start = None

            # Ha a sor végéig eljutottunk, és még mindig tart a fekete blokk
            if count >= 20 and start is not None:
                frame[:start + 10, x] = 0
                hsv[:start + 10, x] = 0
        ...
        if len(orange_cnts) > 0:
            cv2.drawContours(frame, orange_cnts, -1, (0, 255, 0), -1)
            mask_bool = orange_mask > 0
            print(hsv[mask_bool])
